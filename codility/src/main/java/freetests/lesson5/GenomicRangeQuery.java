...
public class GenomicRangeQuery {
    ...
    public int[] solution(String S, int[] P, int[] Q) {
        ...
        int rangeSize = S.length();

        int[][] bitSet = new int[4][rangeSize + 1];
        int sumA = 0;
        int sumC = 0;
        int sumG = 0;
        int sumT = 0;
        ...
        return findMinImpactFactors(bitSet, P, Q, M);
    }

    private int[] findMinImpactFactors(int[][] bitSet, int[] P, int[] Q, int size) {
        int[] minImpactFactors = new int[size];

        for (int i = 0; i < size; i++) {
            int min = P[i];
            int max = Q[i] + 1;
            if (bitSet[0][max] - bitSet[0][min] > 0) {
                minImpactFactors[i] = 1;
            } else if (bitSet[1][max] - bitSet[1][min] > 0) {
                minImpactFactors[i] = 2;
            } else if (bitSet[2][max] - bitSet[2][min] > 0) {
                minImpactFactors[i] = 3;
            } else if (bitSet[3][max] - bitSet[3][min] > 0) {
